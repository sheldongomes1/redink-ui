...
function SparkPanel({ trend, valueKey, color, label, currentVal, gradId, isConviction = false }: any) {
  if (!trend || trend.length < 2) {
    return <div style={{ color: '#9ca3af', fontSize: 13, padding: '32px 0', textAlign: 'center' }}>No history available.</div>;
  }
  const vals: number[] = trend.map((d: any) => d[valueKey]).filter((v: number | null) => v != null);
  if (vals.length < 2) {
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            ...
            <span style={{ fontSize: 12, color: isUp ? '#16a34a' : '#dc2626', fontWeight: 500 }}>
              {isUp ? '▲' : '▼'} {Math.abs(delta).toFixed(isConviction ? 1 : 2)} vs {trend[0].q}
            </span>
          </div>
        </div>
        <div style={{ fontSize: 11, color: '#9ca3af' }}>{trend.length} quarters</div>
      </div>
      <Sparkline data={trend} valueKey={valueKey} width={524} height={100} color={color} gradId={gradId} yAxis={true} formatY={fmtY} />
  ...
  return (
    <ModalShell title={`${row.ticker} — Conviction History`} subtitle={`${row.company_name} · ${row.pattern_name || ''}`} onClose={onClose}>
      {loading ? (
        ...
        <>
          <SparkPanel trend={trend} valueKey="score" color="#635bff" label="Conviction Score Over Time" currentVal={row.conviction_score} gradId="conv-grad" isConviction={true} />
...
function LeftPanel({ rows, selected, onSelect, reviews }: {
  rows: AnomalyListRow[];
    ...
    <div style={{ width: 272, flexShrink: 0, borderRight: '1px solid #f3f4f6', display: 'flex', flexDirection: 'column', background: '#fff' }}>
      <div style={{ padding: '18px 20px 12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 700, letterSpacing: '-0.01em', color: '#111827' }}>RedInk</span>
        </div>
        <input
          type="text"
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#fff', overflow: 'hidden' }}>
      ...
      <div className="panel-scroll section-fade" style={{ flex: 1, padding: '0 32px 32px' }}>
        ...
        <div style={{ paddingTop: 28 }}>
          ...
          <p style={{ fontSize: 14, lineHeight: 1.7, color: '#374151', maxWidth: 680 }}>{row.explanation_brief}</p>
          {row.cited_passage && (
            <div style={{ marginTop: 18, padding: '14px 18px', background: '#f9fafb', borderLeft: '3px solid #635bff', borderRadius: '0 8px 8px 0', maxWidth: 640 }}>
        ...
        {/* Section 2: What Triggered It */}
        <div>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 16 }}>
            <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#9ca3af' }}>What Triggered It</div>
            <button onClick={openDriverTrend} style={{ fontSize: 12, color: '#635bff', background: 'none', border: 'none', fontWeight: 500, padding: 0, letterSpacing: '-0.01em', cursor: 'pointer' }}>View history →</button>
          </div>
            ...
            {drivers.map((d, i) => {
              ...
              return (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <div style={{ minWidth: 26, fontSize: 14, color: isPos ? '#f59e0b' : '#dc2626', textAlign: 'center' }}>{isPos ? '↑' : '↓'}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 5 }}>
                      <span style={{ fontSize: 13, color: '#374151', fontWeight: 500 }}>{DRIVER_LABELS[d.key as string] || d.key}</span>
                      <div style={{ display: 'flex', gap: 8, alignItems: 'baseline' }}>
                        <span style={{ fontSize: 13, fontWeight: 600, color: isPos ? '#d97706' : '#dc2626', fontVariantNumeric: 'tabular-nums' }}>
                          {isPos ? '+' : ''}{v.toFixed(2)}
                        </span>
                        <span style={{ fontSize: 10, color: '#9ca3af', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{band}</span>
                      </div>
                    </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 18, flexWrap: 'wrap' }}>
            {row.pattern_name && (
              <span style={{ fontSize: 11, color: '#6b7280', background: '#f9fafb', border: '1px solid #e5e7eb', padding: '4px 10px', borderRadius: 6 }}>{row.pattern_name}</span>
            )}
            {row.beneish_manipulation_flag && (
              <span style={{ fontSize: 11, color: '#c2410c', background: '#fff7ed', border: '1px solid #fed7aa', padding: '4px 10px', borderRadius: 6 }}>
                ⚠ Beneish manipulation flag · M-score {row.beneish_m_score?.toFixed(1)}
              </span>
            )}
          </div>
        </div>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 16 }}>
            <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#9ca3af' }}>Conviction</div>
            <button onClick={openTrend} style={{ fontSize: 12, color: '#635bff', background: 'none', border: 'none', fontWeight: 500, padding: 0, letterSpacing: '-0.01em', cursor: 'pointer' }}>View history →</button>
          </div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 20 }}>
            <span style={{ fontSize: 36, fontWeight: 700, color: t.fg, letterSpacing: '-0.03em' }}>{row.conviction_score.toFixed(1)}</span>
            <span style={{ fontSize: 14, color: '#9ca3af' }}>/ 100</span>
          </div>
          <div style={{ maxWidth: 460, display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              { label: 'Statistical Anomaly',        val: row.pillar_anomaly,      color: '#635bff' },
              { label: 'Earnings Quality (Beneish)', val: row.pillar_earnings,     color: '#f59e0b' },
              { label: 'Narrative Signal (MD&A)',    val: row.pillar_transparency, color: '#10b981' },
            ].map(p => {
              ...
              return (
                <div key={p.label}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#374151', fontVariantNumeric: 'tabular-nums' }}>{p.val.toFixed(1)}</span>
                  </div>
        ...
        <div>
          ...
          {row.investigation_path ? (
            <>
              <p style={{ fontSize: 14, lineHeight: 1.7, color: '#374151', maxWidth: 680 }}>{row.investigation_path}</p>
              {row.key_question && (
                <div style={{ marginTop: 18, padding: '14px 18px', background: '#fff', borderLeft: '3px solid #635bff', maxWidth: 640 }}>
      ...
      <div style={{ padding: '14px 32px', borderTop: '1px solid #f3f4f6', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
        {[
          ...
          return (
            <button key={btn.key} onClick={() => onReview(row, btn.key)} style={{
        ...
        <a href={row.filing_url || '#'} target="_blank" rel="noopener noreferrer" style={{
...
export default function App() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      <LeftPanel rows={rows} selected={selected} onSelect={setSelected} reviews={reviews} />
      <RightPanel
        row={selectedDetail}
